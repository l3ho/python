...
class PolicyAgent(object):
    def __init__(self, ALPHA, GAMMA=0.95, n_actions=5, fc1=256, input_shape=(600,600), channels=1,
                 chkpt_file='tmp/checkpoints/policy.ckpt', gpu={'GPU':1}):
        self.lr = ALPHA
        self.gamma = GAMMA
    ...
    def build_net(self):
        with tf.variable_scope('parameters'):
            self.input = tf.placeholder(tf.float32, shape=[None, self.input_height, self.input_width, self.channels], name='input')
        ...
        with tf.variable_scope('fc2'):
            dense2 = tf.layers.dense(dense1, units=self.n_actions, kernel_initializer=tf.initializers.glorot_uniform())
            self.actions = tf.nn.softmax(dense2, name='actions')

        with tf.variable_scope('loss'):
            negative_log_prob = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=dense2, labels=self.label)
            self.loss = tf.reduce_mean(negative_log_prob*self.G)

        with tf.variable_scope('train'):
            self.train_op = tf.train.RMSPropOptimizer(self.lr, decay=0.99, momentum=0.0, epsilon=1e-6).minimize(self.loss)
    ...
    def learn(self):
        state_memory = np.array(self.state_memory).reshape((-1, self.input_height, self.input_width, self.channels))
